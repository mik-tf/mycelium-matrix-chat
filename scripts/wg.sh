...
# Fetch WireGuard config path from Terraform outputs
echo "Fetching WireGuard config from Terraform..."
terraform_output=$(tofu -chdir="$DEPLOYMENT_DIR" show -json)

# Extract WireGuard config file path
wg_config_path=$(jq -r '.values.outputs.wireguard_config_path.value' <<< "$terraform_output")

# Read the WireGuard configuration from the file
if [ ! -f "$wg_config_path" ]; then
    echo "ERROR: WireGuard config file not found at $wg_config_path"
    exit 1
fi
wg_config=$(cat "$wg_config_path")

# Write WireGuard configuration to a file
WG_CONFIG_FILE="/etc/wireguard/mmc.conf"
echo "$wg_config" | sudo tee "$WG_CONFIG_FILE" > /dev/null

# Bring down the WireGuard interface if it's up
sudo wg-quick down mmc 2>/dev/null || true

# Bring up the WireGuard interface
sudo wg-quick up mmc
